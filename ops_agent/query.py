...
SYNONYMS = {
    "卡起来了": "加密卡 服务启动失败 初始化错误 KmPermitServer",
    "卡起不来": "加密卡 服务启动失败 初始化错误 KmPermitServer",
    "加密卡": "加密卡 服务启动失败 连接不上 授权 文件 进程 端口 防火墙",
    "连不上卡": "加密卡 连接不上 端口 防火墙 客户端模式",
    "转不出来": "KMVue DDB 转换失败 运行环境 日志",
    "出不了xml": "DDB 生成 XML 失败 配置 路径 权限 日志",
...
def rewrite_query(question: str, attachment_context: str = "") -> str:
    ...
    if "kmvue" in lower and "加密卡" in question:
        expanded.append("KMVue Linux kmconvertservice 运行环境 加密卡连接不上")
...
def _important_attachment_terms(text: str) -> str:
    lowered = text.lower()
    terms = []
    for marker in ("error", "exception", "failed", "fail", "wrong", "cannot", "denied", "timeout", "missing", "not found"):
